...
def test_handle_ping_replies_with_pong(skill):
    ...
    assert sent.data["skill_id"] == skill.skill_id
    assert sent.data["collection"] == "Andersen's Fairy Tales"


def test_handle_ping_does_not_touch_the_index(skill):
    """Pong is meant to be cheap - no index lookup, unlike search."""
    skill.index = None  # would blow up if handle_ping tried to use it
